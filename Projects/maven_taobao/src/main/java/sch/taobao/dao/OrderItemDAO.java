    ...
    public List<OrderItem> listByUser(int uid, int start, int count) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Product product = new ProductDAO().get(pid);
                if(-1!=oid){//如果对应订单项生成订单
                    Order order= new OrderDAO().get(oid);//取出订单对象
                    bean.setOrder(order);//填充订单项
                }

                User user = new UserDAO().get(uid);//取出对应用户
                //填充订单项属性
                bean.setProduct(product);
                bean.setUser(user);
                bean.setNumber(number);
